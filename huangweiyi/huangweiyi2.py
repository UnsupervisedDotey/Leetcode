...
if __name__ == "__main__":
    ...
    # 新建excel
    out_df = pd.DataFrame(columns=list(patient.columns)+["hwy_num"])
    print("len out_df", len(patient))

    for row1 in patient.iterrows():
        print("hwy_num", row1[0])
        row1[1]["hwy_num"] = serial_num_dict.get(row1[1]["variant type"], "")
        row_Data = pd.Series(row1[1])
        out_df = out_df.append(row_Data, ignore_index=True)
    ...
    for row2 in identifier.iterrows():
        print("Identifier", row2[0])
        k_id_v_all_hwy_num.setdefault(row2[1]['Identifier'], [])
        k_id_v_all_hwy_num[row2[1]['Identifier']].append(row2[1]['hwy_num'])
    ...
    for row3 in identifier.iterrows():
        print("hwy_num_group", row3[0])
        row3[1]["hwy_num_group"] = ",".join(k_id_v_all_hwy_num.get(row3[1]["Identifier"], []))
        row_Data = pd.Series(row3[1])
        out_df_group = out_df_group.append(row_Data, ignore_index=True)
